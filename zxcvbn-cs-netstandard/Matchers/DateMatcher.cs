...
using Match = Zxcvbn_cs.Models.MatchTypes.Match;
...
namespace Zxcvbn_cs.Matchers
{
    ...
    /// this matcher.</para>
    /// </summary>
    public sealed class DateMatcher : IMatcher
    {
  ...
  ( \d{1,2} )                         # month or day";
        ...
        {           
            ...
            MatchCollection possibleDates = Regex.Matches(password, "\\d{4,8}"); // Slashless dates
        ...
        private double CalculateEntropy(string match, int? year, bool separator)
        {
            ...
            double entropy = year < 100 ? Math.Log(31 * 12 * 100, 2) : Math.Log(31 * 12 * 119, 2);
        ...
        private Boolean IsDateWithYearType(string match, bool suffix, int yearLen)
        {
            ...
            return IsYearInRange(year) && IsDayMonthString(match.Substring(yearLen, match.Length - yearLen));
        }
